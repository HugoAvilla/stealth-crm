...
async function check() {
    ...
    const { data: roles, error: e2 } = await supabase.from('user_roles').select('*').limit(10);
    console.log("RECENT PROFILES:", JSON.stringify(users, null, 2));
    console.log("ROLES RESULT:", JSON.stringify(roles, null, 2));
    console.log("ERRORS:", e1, e2);
}
