...
import { NewMaterialModal } from "@/pages/Estoque/components/NewMaterialModal";
import { StockEntryModal } from "@/pages/Estoque/components/StockEntryModal";
import { StockExitModal } from "@/pages/Estoque/components/StockExitModal";
import { ConsumptionRulesModal } from "@/pages/Estoque/components/ConsumptionRulesModal";
import { MaterialDetailsModal } from "@/pages/Estoque/components/MaterialDetailsModal";
import { CloseRollModal } from "@/pages/Estoque/components/CloseRollModal";
import { MarginErrorModal } from "@/pages/Estoque/components/MarginErrorModal";
...
export function Principal() {
  ...
  const handleMarginError = (material: Material) => {
    setSelectedMaterial(material);
  ...
  const renderMaterialTable = (items: Material[]) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {items.map((material) => {
        ...
        const isLowStock = !material.is_open_roll && (material.current_stock || 0) <= (material.minimum_stock || 1);

        return (
          <Card
            key={material.id}
            className={cn(
              "bg-card/50 p-4 flex flex-col justify-between space-y-3 transition-all duration-300",
              isLowStock
                ? "border-red-500/80 shadow-[0_0_15px_rgba(239,68,68,0.15)] hover:border-red-500 hover:shadow-[0_0_20px_rgba(239,68,68,0.25)]"
          ...
          >
            ...
            <div className="flex flex-col gap-2 pt-2">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleMarginError(material)}
                className="w-full h-8 text-xs border-orange-500/20 text-orange-600 hover:bg-orange-500/5"
              >
                <Scissors className="mr-1 h-3.5 w-3.5 text-orange-500" /> Ajustar Margem de Erro
              </Button>
            </div>
          </Card>
        );
  ...
  return (
    <div className="space-y-6">
      ...
      />
      <MarginErrorModal
        open={showMarginError}
        onOpenChange={setShowMarginError}
        material={selectedMaterial}
        onSuccess={() => {
