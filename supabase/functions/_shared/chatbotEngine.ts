/**
 * ⚠️  MIRROR of `src/lib/chatbot/engine.ts` — keep the two files in sync.
 * The Supabase CLI only uploads code under `supabase/functions/`, so edge
 * functions cannot import from `src/`. This is a byte-for-byte copy consumed by
 * the Deno edge functions; the canonical, unit-tested source lives in `src/`.
 *
 ...
 *   - the WhatsApp webhook edge function (via this mirror)
 *
